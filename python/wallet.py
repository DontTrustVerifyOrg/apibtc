...
class Wallet:
    ...
    def listchannels(self, activeOnly: bool) -> Any:
        """
        Retrieves a list of Lightning Network channels associated with the LND node. This endpoint provides detailed information about each channel, including its capacity, balance, and current state. It can be used to monitor the node's connectivity and liquidity within the Lightning Network. The response includes both active and inactive channels, depending on the 'activeOnly' parameter.

        Args:
            activeOnly: If true, returns only active channels. If false, returns all channels including inactive ones.

        Returns:
            ChannelArrayResult containing list of channel information
        """
        api_url = f"{self.base_url}/listchannels"
        response = requests.get(url=api_url, params={"authToken": self._create_authtoken(), "activeOnly": activeOnly})
        response.raise_for_status()
        return self.parse_response(response.json())

    def listclosedchannels(self) -> Any:
        """
        Retrieves a list of closed Lightning Network channels associated with the LND node. This endpoint provides detailed information about each closed channel, including its capacity, closing transaction, and settlement details. It can be used to review the history of closed channels and analyze past network connections.

        Returns:
            ChannelCloseSummaryArrayResult containing list of closed channel information
        """
    ...
    def estimatefee(self, address: str, satoshis: int) -> Any:
        """
        Provides a comprehensive fee estimation for a potential payout that includes both the Bitcoin transaction fee and the Lightning Network channel closing fee. This endpoint calculates the estimated on-chain transaction fee based on the provided destination address and amount, as well as the potential fee for closing a Lightning Network channel. It returns the total Bitcoin transaction fee in satoshis, the fee rate in satoshis per virtual byte, and the estimated channel closing fee. This information is crucial for users to understand the full cost implications of their payout, including the possibility of needing to close a channel to fulfill the payout.

        Args:
            address: The destination Bitcoin address for the potential payout transaction. This should be a valid Bitcoin address where the funds would be sent.
            satoshis: The amount of the potential payout, specified in satoshis (1 satoshi = 0.00000001 BTC). This value must be a positive integer representing the exact amount of the intended payout.

        Returns:
            FeeEstimateRetResult containing fee estimation details
        """
    ...
    def getpayout(self, payoutId: str) -> Any:
        """
        ...
        Returns:
            PayoutRecordResult containing the payout record
        """
        api_url = f"{self.base_url}/getpayout"
        response = requests.get(url=api_url, params={"authToken": self._create_authtoken(), "payoutId": payoutId})
        response.raise_for_status()
        return self.parse_response(response.json())
    
    def addinvoice(self, satoshis: int, memo: str, expiry: int) -> Any:
        """
        Creates and returns a new Lightning Network invoice for receiving payments. This endpoint allows users to generate payment requests with customizable amount, memo, and expiration time. The created invoice can be shared with payers to facilitate Lightning Network transactions.
        ...
        Args:
            ...
            memo: An optional memo or description for the invoice. This can be used to provide additional context or details about the payment to the payer. The memo will be included in the encoded payment request.
            expiry: The expiration time for the payment request, in seconds. After this duration, the invoice will no longer be valid for payment.

        Returns:
            InvoiceRecordResult containing the created invoice details
        """
